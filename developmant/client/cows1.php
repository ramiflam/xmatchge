...
<h1><?php echo $updatePhrase;?></h1>
	 ...
	 $query="SELECT * FROM `local_cows` where User_Id = '$username' and Farm= '$userfarm' and match_status=1 and sex=2 order by $order limit 11";
        }
        else
        {
         $query="SELECT * FROM `local_cows` where User_Id = '$username' and Farm= '$userfarm' order by $order";
        }
        //echo  $query;
