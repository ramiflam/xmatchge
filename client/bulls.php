...
<input type="text" name="search" id="search" placeholder=" Bull Name/Number" onkeypress=" searchKeyPress(event)" searchWord="<?php echo $searchNum['bull_no'] ?>">
...
<button class="submit" type="submit" id="bull2" value="<?php if(isset($_POST['showall'])) {echo 'false';} else {echo 'true';}?>" > Active?</button>
	...
	<div id="table">
        ...
        <td class="secttl"><a href="bulls.php?order=bull_name"  style="text-decoration:none;color:#fff" >Name<a><br>
        <img id="arrow"src="/assets/arrow.png"></td>
        ...
        <td class="secttl"><a href="bulls.php?order=Match_status"  style="text-decoration:none;color:#fff" >Active?<a></td>
        <td class="secttl" id="color"><a href="bulls.php?order=Heifer_status"  style="text-decoration:none;color:#fff" >For Calf<a></td>
        <td class="secttl"><a href="bulls.php?order=Planned_usage"  style="text-decoration:none;color:#fff" >Planned %<a>
        <img id="arrow"src="/assets/arrow.png"></td>
        <td class="secttl"><a href="bulls.php"  style="text-decoration:none;color:#fff" >Actual %<a></td>
        <td class="secttl"  id="last" ><a href="bulls.php?order=Limited"  style="text-decoration:none;color:#fff" >Limited<a></td>
        <td class="secttl" style="border-right: 1px solid #fff;"><a href="bulls.php?order=Actuall_inseminations"  style="text-decoration:none;color:#fff" >Actual Insem. No.<a></td>
        <td class="secttl"><a href="bulls.php?order=From_insemination"  style="text-decoration:none;color:#fff" >From Service<a></td>
        <td class="secttl" ><a href="bulls.php?order=To_insemination"  style="text-decoration:none;color:#fff" >To Service<a></td>
...
$offset = 0;
	 ...
	 $query="SELECT b.bull_no, bull_name, CVM, KG_ECM, Planned_usage, Actuall_inseminations, Usage_order, General_size, General_udder, Teats_location, Udder_depth, General_legs, Pelvis_stucture, Fat_percentage, Protein_percentage, MGS, PGS, SCC, sire, pic_link, breed,
...
if(!empty($_REQUEST['search'])) {
   /*if(isset($_POST['find'])){*/
	...
	$searchWords= explode(",", $search);
 	...
 	$query= $query."ORDER BY $order FIELD(breed, 1,39) DESC, breed , bull_no  LIMIT $offset, $_count";
	  }
	  else if(!isset($_POST['showall']) or $showAll=='false')
	  	{
	  $query= $query." WHERE (ISNULL(u.match_status) AND b.match_status=1) OR (u.match_status=1  AND u.userID = 'Avi') ORDER BY $order FIELD(breed, 1,39) DESC, breed , bull_no LIMIT $offset, $_count";
	  }
	//echo $query;
	  ...
	  <div class="tooltip_sectititle">
       ...
       <!-- <td>
        ...
        <option value="sx">SX</option>
           ...
           <td>
          <select name="orderby" >
          <option value="<?php echo $row['Order_by_Fertility'] ;?>"><?php echo $row['Order_by_Fertility']; ?></option>
  <?php  
   for ($x = 0; $x <= 10; $x++) { ?>
