...
$set3_geno =getMsg($db, $browserLanguage, 'set3_geno');

$query="SELECT * FROM `user_settings` WHERE `user_id`='$username' and FarmName='$userfarm'";
...
<script>
 ...
 function showVal(newVal,elmName, elmNameVal){
  	 var slider3Val = document.getElementById('slider1').getAttribute('max') - document.getElementById('slider1').value - document.getElementById('slider2').value - document.getElementById('slider4').value;
	  ...
	  if(slider3Val >= 0){
	  document.getElementById('valBox3').innerHTML= Math.round(slider3Val/document.getElementById('slider1').getAttribute('max') * 100) + '%';
	  document.getElementById('valBox31').innerHTML='('+ slider3Val + ')';
...
<form  name="set"  method="post"  action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ; ?>" user_id="<?php echo $username?>" farm="<?php echo $userfarm?>">
    <h1> <?php echo $GenPhrase; ?></h1>
 
    <table>
        <tr>
            <th><?php echo $set3_young ?></th>
            <td>
...
<tr>
            <th><?php echo $set3_geno ?></th>
            <td>
            <span class="sliderVal" id="valBox4"><?php echo round($row["Bulls_usage_genomic_bulls"]/$row2[count]*100) ?>%</span>
            <span class="sliderVal2" id="valBox41">(<?php echo $row["Bulls_usage_genomic_bulls"] ?>)</span>
         <input type="range" id="slider4" value="<?php echo $row["Bulls_usage_genomic_bulls"] ?>" name="Bulls_usage_genomic_bulls" min=0 max=<?php echo $row2[count] ?> oninput="showVal(this.value, 'valBox4', 'valBox41')" onchange="showVal(this.value, 'valBox4', 'valBox41')" />       
            </td>
        </tr>
                  <tr>
            <th><?php echo $set3_test ?></th>
            <td>
            <span class="sliderVal" id="valBox3"><?php echo  round(($row2[count]-$row["Bulls_usage_young_bulls"]-$row["Bulls_usage_meat_bulls"]-$row["Bulls_usage_genomic_bulls"])/$row2[count]*100)?>%</span>
            <span class="sliderVal2" id="valBox31">(<?php echo $row2[count]-$row["Bulls_usage_young_bulls"]-$row["Bulls_usage_meat_bulls"]-$row["Bulls_usage_genomic_bulls"] ?>)</span>
        <input type="range" id="slider3" value="<?php echo $row2[count]-$row["Bulls_usage_young_bulls"]-$row["Bulls_usage_meat_bulls"]-$row["Bulls_usage_genomic_bulls"]?>" name="" min=0 max=<?php echo $row2[count] ?> oninput="" onchange="" disabled/>
            </td>
        </tr>
<tr>
    <th><?php echo $set3_lact ?></th>
    <td>
<select name="Meat_bulls_lactation_no">
  ...
  <option value="10" <?php if($row['Meat_bulls_lactation_no']=="10") echo selected?> >10</option>
    ...
    <th><?php echo $set3_up ?></th>
    <td >
<select id="upsidedown" title="Ask an expert!" onchange="if(this.value!='10%') document.getElementById('tooltip').style.display = 'block'; else document.getElementById('tooltip').style.display = 'none';">
...
<div class="tooltip" id="tooltip">Ask an expert!</div>
  ...
  <tr>
    ...
    <td>
     <input type="text" name="Bulls_Forbidden" placeholder="Raffaello" required value="<?php echo $row['Bulls_Forbidden_for_Insemination']?>">   
    </td>
 </tr>
    </table><br>
 ...
 //function updateDB()
  $('#updateDB5').click( function(){
 	    var data = {
 	    	...
 	    	Bulls_Forbidden:$(this).parent().find('[name=Bulls_Forbidden]').val(),
 	    	Bulls_usage_genomic_bulls: $(this).parent().find('[name=Bulls_usage_genomic_bulls]').val()
